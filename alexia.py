...
list_of_linkedin=["لينكدان ","لينكد","لينك"]
list_of_git=["جيت","جيت هاب"]
list_of_chat=["تشات ","شات جبت","شات"]
list_of_end=["انهاء","انهي","انها"]
...
def run():
    ...
    while x:
        command=listen()
        if any(word in command for word in list_of_end):
                x=False
            ...
            speak("طبعا اعرفك يا ابراهيم ")
        elif any(word in command for word in list_of_linkedin):
            webbrowser.open("https://www.linkedin.com/in/ibrahim-elmsery-929099220/")
        elif any(word in command for word in list_of_git):
            webbrowser.open("https://github.com/ibrahim-reda-2001?tab=repositories")
